...
class WC_Memberships_Integration_Subscriptions_Discounts {
	...
	public function __construct() {
		...
		// handle Subscriptions-specific discount handling if there's a member logged in
		if ( ! ( is_admin() && ! is_ajax() ) && wc_memberships_is_user_member( get_current_user_id() ) ) {
	...
	public function get_subscription_product_price_html( $html_price, $product ) {
		...
		if (      $product->is_type( array( 'subscription', 'variable-subscription', 'subscription_variation' ) )
		     &&   wc_memberships_product_has_member_discount( $product->get_id() )
	...
	public function maybe_adjust_product_sign_up_fee( $sign_up_fee, $subscription_product ) {
		...
		if ( ! isset( $this->sign_up_fee[ $subscription_product->get_id() ] ) ) {
			...
			if ( wc_memberships_user_has_member_discount( $subscription_product ) && 'yes' === get_option( 'wc_memberships_enable_subscriptions_sign_up_fees_discounts', 'no' ) ) {
