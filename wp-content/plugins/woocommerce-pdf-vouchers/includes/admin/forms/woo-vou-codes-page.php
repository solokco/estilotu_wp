			...
			<h2 class="nav-tab-wrapper woo-vou-h2">
				<?php
					...
					$voucher_code_tabs	= array(
											'purchased' => array(
																	'name' => __( 'Purchased Voucher Codes', 'woovoucher' ),
																	'path' => WOO_VOU_ADMIN .'/forms/woo-vou-purchased-list.php'
																),
											'used' 		=> array(
																	'name' => __( 'Used Voucher Codes', 'woovoucher' ),
																	'path' => WOO_VOU_ADMIN .'/forms/woo-vou-used-list.php'
																),
											'expired' 	=> array(
																	'name' => __( 'Unused Voucher Codes', 'woovoucher' ),
																	'path' => WOO_VOU_ADMIN .'/forms/woo-vou-purchased-list-expire.php'
																),
											);
						
						// if enable then disply partially used tab
						$enable_partial_redeem = get_option( 'vou_enable_partial_redeem' );
						
						if( $enable_partial_redeem == "yes" ) {
							$offset = 1;
							$voucher_code_tabs = array_slice( $voucher_code_tabs, 0, $offset, true ) +
														array( 'partially_redeemed' => array(
																	'name' => __( 'Partially Used Voucher Codes', 'woovoucher' ),
																	'path' => WOO_VOU_ADMIN .'/forms/woo-vou-partially-redeemed-list.php'
																) ) +
															array_slice( $voucher_code_tabs, $offset, NULL, true );
						}
					...
					if( !empty( $voucher_code_tabs ) ) {
						...
						foreach ( $voucher_code_tabs as $tab => $tab_details ) {
							...
							?>
					        <a class="nav-tab<?php echo $selected;?>" href="<?php echo $vou_code_page_url;  ?>"><?php echo !empty( $tab_details['name'] ) ? $tab_details['name'] : '';?></a>
							<?php
						}
					}?>
		    </h2><!--nav-tab-wrapper-->
			 ...
			 <div class="woo-voucher-code-content">
			 	...
			 	<?php
					...
					if( !empty( $voucher_code_tabs[$curr_tab]['path'] ) && file_exists( $voucher_code_tabs[$curr_tab]['path'] ) ) {
						...
						echo __( 'File not found.', 'woovoucher' );
					}
				?>
			 <!--end of tabs panels-->
			 </div>
