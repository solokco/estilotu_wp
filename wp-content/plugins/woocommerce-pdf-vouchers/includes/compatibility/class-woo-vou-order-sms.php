	...
	function woo_vou_pharse_sms_body_container( $sms_body, $order_id ) {
		...
		foreach( $order_items as $item_key => $item_val ) {
			...
			if( !$_product ) {//If product deleted
					...
					//Get download files
					$download_file_data	= $woo_order->get_item_downloads( $item_val );
			}
			...
			$voucher_details[] = __( 'Product Name:', 'woovoucher' ) . $item_val['name'] . ',' .
								 __( 'Voucher Code:', 'woovoucher' ) . $item_val['woo_vou_codes'] . ',' .
								 __( 'Price:', 'woovoucher' ) . $product_price . ',' .
								 __( 'Expire date:', 'woovoucher' ) . $expire_date . ',' .
								 __( 'Link:', 'woovoucher' ) . $links;
		}
